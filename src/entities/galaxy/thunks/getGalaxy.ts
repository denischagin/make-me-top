...
import { DEFAULT_ERROR_MESSAGE, DEFAULT_GALAXY_ID } from '../model/constants';
...
>(FETCH_GALAXY, async (payload, { rejectWithValue }) => {
    try {
        const { galaxyId = DEFAULT_GALAXY_ID } = payload;

        const { data } = await instance.get<GalaxyResponseInterface>(
